...
class RedisMCP(BaseMCPServer):
    ...
    def _handle_hincrby(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error incrementing hash field {field} for key {key}: {e}")
    ...
    def _handle_lrange(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"key": key, "start": int(start), "stop": int(stop), "values": values}
        except Exception as e:
    ...
    def _handle_zadd(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"key": key, "value": value, "score": float(score), "added": result > 0}
        except Exception as e:
    ...
    def _handle_zrange(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if withscores:
                values = self.redis_client.zrange(key, int(start), int(stop), withscores=True)
                # Convert to a more JSON-friendly format
                result = []
                for value, score in values:
                    result.append({"value": value, "score": score})
                return {"key": key, "start": int(start), "stop": int(stop), "values": result}
            else:
    ...
    def _handle_zrevrange(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if withscores:
                values = self.redis_client.zrevrange(key, int(start), int(stop), withscores=True)
                # Convert to a more JSON-friendly format
                result = []
                for value, score in values:
                    result.append({"value": value, "score": score})
    ...
    def set_hash(self, key: str, field_values: Dict[str, Any]) -> bool:
        ...
        for field, value in field_values.items():
            result = self.fetch_data("hset", {"key": key, "field": field, "value": value})
            if "error" in result:
                success = False
        return success
    ...
    def increment_hash_value(self, key: str, field: str, amount: int = 1) -> int:
        ...
        """
        return self.fetch_data("hincrby", {"key": key, "field": field, "amount": amount}).get("value", 0)
