...
class UnusualWhalesMCP(BaseDataMCP):
    ...
    def _initialize_client(self) -> Dict[str, Any]:
        ...
        try:
            ...
            api_key = self.config.get("api_key") or os.environ.get("UNUSUAL_WHALES_API_KEY")
            base_url = self.config.get("base_url", "https://api.unusualwhales.com").rstrip("/")
            
            if not api_key:
                self.logger.warning("No Unusual Whales API key provided - API calls will fail")
    ...
    def _verify_api_connectivity(self, api_key: str, base_url: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"API connectivity failed with status {response.status_code}: {response.text}")
                self.logger.error(f"Please check the API base URL ({base_url}) and API key")
                return False
    ...
    def _execute_api_request(self, path: str, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def get_options_flow(self, ticker: str, limit: int = 50) -> Dict[str, Any]:
        ...
        Returns:
            Options flow data
        """
        return self.fetch_data("options_flow", {
            "ticker": ticker,
            "limit": str(limit)
        })
    ...
    def get_unusual_activity(self, ticker: str, limit: int = 50) -> Dict[str, Any]:
        ...
        """
        return self.fetch_data("unusual_activity", {
            "ticker": ticker,
            "limit": str(limit)
        })
    ...
    def get_dark_pool(self, ticker: str, limit: int = 50) -> Dict[str, Any]:
        ...
        """
        return self.fetch_data("dark_pool", {
            "ticker": ticker,
            "limit": str(limit)
        })
    ...
    def get_global_flow_alerts(self, limit: int = 50) -> Dict[str, Any]:
        ...
        """
        return self.fetch_data("global_flow_alerts", {
            "limit": str(limit)
        })
    
    def search_flow_by_criteria(self, criteria: Dict[str, Any], limit: int = 50) -> Dict[str, Any]:
        """
        ...
        """
        return self.fetch_data("flow_by_criteria", {
            "criteria": criteria,
            "limit": str(limit)
        })
