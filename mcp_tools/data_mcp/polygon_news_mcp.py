...
class PolygonNewsMCP(BaseDataMCP):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info("PolygonNewsMCP initialized with %d endpoints", len(self.endpoints))
    ...
    def _initialize_endpoints(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "latest_news": {
                ...
                "category": "news",
                "required_params": [],
                "optional_params": ["limit", "order", "sort", "published_utc.gt", "published_utc.lt"],
                "default_values": {
            ...
            "news_search": {
                ...
                "optional_params": ["ticker", "limit", "order", "sort", "published_utc.gt", "published_utc.lt"],
                "default_values": {
                    "limit": str(self.default_results_limit),
                    "order": "desc",
                    "sort": "published_utc"
    ...
    def _initialize_sentiment_analyzer(self):
        ...
        try:
            ...
            # In a more sophisticated implementation, we could use a more advanced NLP model
            from nltk.sentiment.vader import SentimentIntensityAnalyzer
            self.sentiment_analyzer = SentimentIntensityAnalyzer()
            self.logger.info("Initialized VADER sentiment analyzer")
        except ImportError:
            self.logger.warning("NLTK VADER not available, using simple keyword-based sentiment analysis")
            # Initialize a dictionary of positive and negative terms for basic sentiment analysis
            self.positive_terms = set([
                'up', 'rise', 'rising', 'gain', 'gains', 'positive', 'profitable', 'growth',
                'growing', 'increase', 'increasing', 'higher', 'improved', 'improving', 'strong',
                'stronger', 'strongest', 'beat', 'beats', 'beating', 'exceed', 'exceeds', 'exceeded',
                'surpass', 'surpasses', 'surpassing', 'outperform', 'outperforms', 'outperforming',
                'record', 'bullish', 'bull', 'buy', 'buying', 'rally', 'rallies', 'rallying',
                'recover', 'recovery', 'recovering', 'boom', 'roar', 'soar'
            ])
            self.negative_terms = set([
                'down', 'fall', 'falling', 'loss', 'losses', 'negative', 'unprofitable', 'decline',
                'declining', 'decrease', 'decreasing', 'lower', 'worsened', 'worsening', 'weak',
                'weaker', 'weakest', 'miss', 'misses', 'missing', 'fail', 'fails', 'failing',
                'underperform', 'underperforms', 'underperforming', 'bearish', 'bear', 'sell',
                'selling', 'slump', 'slumps', 'slumping', 'plunge', 'plunges', 'plunging',
                'crash', 'crashes', 'crashing', 'tumble', 'tumbles', 'tumbling', 'sink'
            ])
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if retry_count <= max_retries:
                        ...
                        self.logger.warning(f"Rate limit exceeded, retrying in {sleep_time}s (attempt {retry_count}/{max_retries})")
                ...
                if retry_count <= max_retries:
                    ...
                    self.logger.warning(f"Request failed, retrying in {sleep_time}s (attempt {retry_count}/{max_retries}): {e}")
                    time.sleep(sleep_time)
                else:
            ...
            if "published_utc" in article:
                ...
                try:
                    ...
                    if delta.days > 0:
                        ...
                        article["published_relative"] = f"{delta.seconds // 3600} hours ago"
                    elif delta.seconds >= 60:
                        article["published_relative"] = f"{delta.seconds // 60} minutes ago"
                    else:
    ...
    def _handle_market_news(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "articles" in latest_news:
            market_articles = [
                article for article in latest_news["articles"]
    ...
    def _handle_sector_news(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # In production, we'd use a more sophisticated mapping of tickers to sectors
        # For now, we'll use the keyword search approach
        params["keywords"] = sector
        return self._handle_news_search(params)
    
    def _execute_polygon_request(self, path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_latest_news(self, limit: int = None, hours_ago: int = None) -> Dict[str, Any]:
        """
