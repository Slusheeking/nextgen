...
class BaseDataMCP(BaseMCPServer):
    ...
    def fetch_data(
        ...
        while attempt < self.error_retry_limit:
            try:
                ...
                self.logger.info(
                    f"Successfully fetched data from {endpoint} in {elapsed:.2f}s"
                )
                return result
                ...
                attempt += 1
                self.logger.warning(
                    f"Attempt {attempt}/{self.error_retry_limit} failed for {endpoint}: {e}"
                )
    ...
    def _execute_endpoint_fetch(
        ...
        raise NotImplementedError(
            "Child classes must implement _execute_endpoint_fetch"
        )
    ...
    def _update_endpoint_stats(
        self, endpoint: str, event: str, elapsed: Optional[float] = None
    ) -> None:
        """
    ...
    def clear_cache(self, endpoint: Optional[str] = None) -> Dict[str, Any]:
        ...
        if endpoint:
            ...
            return {
                "status": "success",
                "message": f"Cleared cache for endpoint: {endpoint}",
    ...
    def get_data_source_status(self) -> Dict[str, Any]:
        ...
        total_successes = sum(
            s.get("successes", 0) for s in self.endpoint_stats.values()
        )
        total_errors = sum(s.get("errors", 0) for s in self.endpoint_stats.values())
        total_cache_hits = sum(
            s.get("cache_hits", 0) for s in self.endpoint_stats.values()
        )
        ...
        cache_hit_rate = (
            (total_cache_hits / (total_requests + total_cache_hits)) * 100
            if (total_requests + total_cache_hits) > 0
            else 0
        )
