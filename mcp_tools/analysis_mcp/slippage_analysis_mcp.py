...
class SlippageAnalysisMCP(BaseMCPServer):
    ...
    def _initialize_endpoints(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "analyze_market_impact": {
                ...
                "category": "execution_quality",
                "required_params": ["pre_trade_prices", "post_trade_prices", "trade_size", "average_volume"],
            ...
            "evaluate_execution_timing": {
                ...
                "category": "execution_quality",
                "required_params": ["execution_price", "price_series", "execution_index"],
    ...
    def _handle_analyze_market_impact(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not pre_trade_prices or not post_trade_prices or trade_size is None or average_volume is None:
            return {"error": "Missing required parameters"}
        ...
        try:
            ...
            impact_result = self._analyze_market_impact(
                pre_trade_prices, post_trade_prices, trade_size, average_volume, time_window
            )
    ...
    def _handle_evaluate_execution_timing(self, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _handle_analyze_liquidity_impact(self, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _calculate_slippage(self, expected_price: float, executed_price: float, 
    ...
    def _analyze_market_impact(self, pre_trade_prices: np.ndarray, 
                             post_trade_prices: np.ndarray,
                             trade_size: float, average_volume: float,
        ...
        volatility_change_percent = volatility_change / pre_trade_std * 100 if pre_trade_std > 0 else 0
    ...
    def _evaluate_execution_timing(self, execution_price: float, 
        ...
        if len(window) > 1:
            ...
            percentile = np.searchsorted(sorted_prices, execution_price) / len(sorted_prices) * 100
        else:
            percentile = 50.0
        ...
        if len(window) > 1:
            if min_price == max_price:
                ...
                timing_score = 100.0 - (abs(execution_price - mean_price) / (max_price - min_price) * 100.0)
        else:
    ...
    def _analyze_liquidity_impact(self, pre_trade_bid_ask: Dict[str, Any],
                                post_trade_bid_ask: Dict[str, Any],
                                side: str, time_window: int = 5) -> Dict[str, Any]:
        """
        ...
        spread_change_percent = spread_change / pre_spread * 100 if pre_spread > 0 else 0
        
        # Calculate size changes
        if side.lower() == 'buy':
            # For buys, we're more interested in ask side
            size_change = post_ask_size - pre_ask_size
            size_change_percent = size_change / pre_ask_size * 100 if pre_ask_size > 0 else 0
            price_change = post_ask - pre_ask
        else:  # sell
            # For sells, we're more interested in bid side
            size_change = post_bid_size - pre_bid_size
            size_change_percent = size_change / pre_bid_size * 100 if pre_bid_size > 0 else 0
            price_change = post_bid - pre_bid
        
        price_change_percent = price_change / (pre_bid if side.lower() == 'sell' else pre_ask) * 100
    ...
    def calculate_slippage(self, expected_price: float, executed_price: float, 
                          side: str, quantity: float = 1.0) -> Dict[str, Any]:
        """
        Calculate execution slippage.
    ...
    def analyze_market_impact(self, pre_trade_prices: List[float], 
                             post_trade_prices: List[float],
                             trade_size: float, average_volume: float) -> Dict[str, Any]:
        """
        Analyze market impact of a trade.
    ...
    def evaluate_execution_timing(self, execution_price: float, 
                                price_series: List[float],
                                execution_index: int) -> Dict[str, Any]:
        """
        Evaluate execution timing optimality.
    ...
    def analyze_liquidity_impact(self, pre_trade_bid_ask: Dict[str, Any],
                               post_trade_bid_ask: Dict[str, Any],
