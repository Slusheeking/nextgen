...
This module implements the trade execution and monitoring component of the FinGPT system.
...
class TradeExecutor: pass
class TradeMonitor: pass
class TradePositionManager: pass
class TradeAnalytics: pass
class TradeModel: pass # Add self-reference for type hinting
...
class TradeModel:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        """
        # Initialize logging
        self.logger = logging.getLogger("fingpt.nextgen_trader")
        ...
        self.peak_detection_mcp = PeakDetectionMCP(self.config.get("peak_detection_config"))
        self.slippage_analysis_mcp = SlippageAnalysisMCP(self.config.get("slippage_analysis_config"))
        self.drift_detection_mcp = DriftDetectionMCP(self.config.get("drift_detection_config"))
    ...
    def _register_functions(self, user_proxy: UserProxyAgent):
        ...
        def execute_market_order(symbol: str, quantity: float, side: str) -> Dict[str, Any]:
        ...
        def execute_limit_order(symbol: str, quantity: float, side: str, limit_price: float) -> Dict[str, Any]:
            return self.executor.execute_limit_order(symbol, quantity, side, limit_price)
        ...
        @register_function(
            ...
            parameter_schema={
                ...
                "order_id": {
                    "type": "string",
                    "description": "Order ID of the executed trade"
                }
            },
            return_type=Dict[str, Any]
        )
        ...
        def get_trade_history(symbol: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    def _register_mcp_tool_access(self, user_proxy: UserProxyAgent):
        ...
        def use_slippage_analysis_tool(tool_name: str, arguments: Dict[str, Any]) -> Any:
    ...
    def execute_trade(self, trade_decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if order_type == 'market':
            ...
            return self.executor.execute_limit_order(symbol, quantity, action, limit_price)
        else:
            return {"status": "failed", "reason": "Invalid order type or missing limit price"}
            ...
            try:
                ...
                if exit_result.get("should_exit", False):
                    ...
                    if sell_result.get("status") == "filled":
                        ...
                        self.notify_decision_model("capital_available", {
...
class TradeExecutor:
    ...
    def execute_market_order(self, symbol: str, quantity: float,
        ...
        """
        self.logger.info(f"Executing {side} market order for {quantity} shares of {symbol}")
        ...
        try:
            ...
            if side == 'buy' and result.get('status') == 'filled':
                self.trade_model.monitor.start_position_monitoring(symbol, result.get('id'))
            ...
            if side == 'sell' and result.get('status') in ['filled', 'partially_filled']:
                filled_qty = float(result.get('filled_qty', 0))
                filled_price = float(result.get('filled_avg_price', 0))
    ...
    def execute_limit_order(self, symbol: str, quantity: float, side: str,
        ...
        Returns:
            Order result dictionary
        """
        self.logger.info(f"Executing {side} limit order for {quantity} shares of {symbol} at {limit_price}")
...
class TradeMonitor:
    ...
    def start_position_monitoring(self, symbol: str, order_id: str) -> Dict[str, Any]:
        ...
        try:
            self.trade_model.redis_mcp.set_json(key, monitor_config)
             ...
             self.logger.error(f"Error storing monitor config for {symbol} in Redis: {e}")
    ...
    def check_exit_conditions(self, symbol: str) -> Dict[str, Any]:
        ...
        config['high_since_entry'] = max(config.get('high_since_entry', current_price), current_price)
        config['low_since_entry'] = min(config.get('low_since_entry', current_price), current_price)
        config['last_update_time'] = datetime.now().isoformat()

        # Calculate key metrics
        entry_price = config.get('entry_price', 0)
        gain_loss_pct = (current_price - entry_price) / entry_price if entry_price != 0 else 0
        high_since_entry = config['high_since_entry']
        drawdown_from_peak = (current_price - high_since_entry) / high_since_entry if high_since_entry != 0 else 0
        ...
        elif self.trade_model.no_overnight_positions and self._is_approaching_market_close():
            exit_result["should_exit"] = True
    ...
    def _load_monitor_config(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if config:
                ...
                config['trailing_stop_pct'] = float(config.get('trailing_stop_pct', 0.01))
    ...
    def _is_approaching_market_close(self) -> bool:
        ...
        try:
            ...
            closing_time_str = clock.get("next_close") or clock.get("closing_at") # Alpaca API might use next_close
            if not closing_time_str:
                 self.logger.warning("Could not determine market closing time.")
                 return False

            # Handle potential timezone differences if needed, assuming UTC or local timezone consistent with Alpaca
            # Alpaca times are typically US/Eastern
            closing_time = pd.Timestamp(closing_time_str, tz='America/New_York').tz_convert('UTC')
            now = pd.Timestamp.utcnow() # Use UTC now

            # Check if within 15 minutes of closing
            time_to_close = (closing_time - now).total_seconds()
            # self.logger.debug(f"Time until market close: {time_to_close / 60:.2f} minutes")
...
class TradePositionManager:
    ...
    def update_daily_usage(self, amount: float) -> float:
        ...
        try:
            ...
            new_usage = self.trade_model.redis_mcp.increment_float(self._daily_usage_key, amount)
            # Set expiry for the key (e.g., 24 hours) to auto-reset daily
            self.trade_model.redis_mcp.expire(self._daily_usage_key, 86400) # 24 * 60 * 60
            self.logger.info(f"Updated daily capital usage by {amount:.2f}. New usage: {new_usage:.2f}")
            return new_usage
        except Exception as e:
...
class TradeAnalytics:
    ...
    def get_trade_history(self, symbol: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            order_ids = self.trade_model.redis_mcp.get_sorted_set_members("trade:orders", 0, -1, reverse=True) # Get all, newest first
            ...
            for order_id in order_ids:
                ...
                if order and (not symbol or order.get('symbol') == symbol):
                    ...
                    order['limit_price'] = float(order.get('limit_price', 0)) if order.get('limit_price') is not None else None
                    order['filled_avg_price'] = float(order.get('filled_avg_price', 0)) if order.get('filled_avg_price') is not None else None
                    orders.append(order)
                    count += 1
    ...
    def calculate_execution_quality(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not filled_at or filled_price is None:
                return {"status": "pending", "reason": "Order not filled yet or missing price"}

            # Convert filled_at to datetime
            filled_time = pd.to_datetime(filled_at) # Assumes ISO format with timezone

            # Get market data for 1 minute before and after execution for tighter analysis
            start_time = filled_time - timedelta(minutes=1)
            end_time = filled_time + timedelta(minutes=1)

            # Format times for Alpaca API (ensure UTC or correct timezone handling)
            ...
            # --- Calculate metrics ---
            prices = [float(bar.get("c", 0)) for bar in bars]
            vwaps = [float(bar.get("vw", 0)) for bar in bars if bar.get("vw") is not None] # Alpaca minute bars have VWAP

            # Calculate VWAP during the execution minute(s)
            exec_minute_bars = [b for b in bars if pd.to_datetime(b.get('t')).floor('min') == filled_time.floor('min')]
            exec_vwap = np.mean([float(b.get('vw', 0)) for b in exec_minute_bars if b.get('vw') is not None]) if exec_minute_bars else None
            ...
            # Calculate slippage from arrival price (e.g., price at the start of the minute)
            arrival_price = float(bars[0].get('o', 0)) if bars else None # Opening price of the first bar
            slippage_arrival = None
            if arrival_price is not None:
                 if side == 'buy':
                    slippage_arrival = (filled_price - arrival_price) / arrival_price * 100
                 else: # sell
                    slippage_arrival = (arrival_price - filled_price) / arrival_price * 100


            # Use slippage analysis MCP for potentially more advanced metrics
            advanced_metrics = {}
            try:
                advanced_metrics = self.trade_model.slippage_analysis_mcp.analyze_slippage({
                    "symbol": symbol,
                    "side": side,
                ...
                self.logger.warning(f"Error calculating advanced slippage metrics via MCP: {e}")
            ...
            self.logger.error(f"Error calculating execution quality for {order_id}: {e}")
    ...
    def calculate_daily_performance(self, date: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Increase limit significantly to capture all potential trades for a day
            trades = self.get_trade_history(limit=5000)

            # Filter trades by date (ensure proper date comparison)
            day_trades = [
                trade for trade in trades
                if trade.get('filled_at') and pd.to_datetime(trade.get('filled_at')).strftime('%Y-%m-%d') == date
            ]
            ...
            buy_volume = sum(t.get('qty', 0) * t.get('filled_avg_price', 0) for t in buy_trades if t.get('filled_avg_price') is not None)
            sell_volume = sum(t.get('qty', 0) * t.get('filled_avg_price', 0) for t in sell_trades if t.get('filled_avg_price') is not None)

            # Calculate P&L (simplified - assumes all positions opened/closed daily or ignores overnight)
            # A more robust P&L requires tracking positions across days.
            pnl = sell_volume - buy_volume

            # Calculate win rate (requires pairing buy/sell trades, more complex)
            # Simplified: count profitable trades (sell price > buy price for pairs)
            # This requires a more sophisticated pairing logic. Placeholder for now.
            win_rate = None
            profitable_trades = 0
            losing_trades = 0
